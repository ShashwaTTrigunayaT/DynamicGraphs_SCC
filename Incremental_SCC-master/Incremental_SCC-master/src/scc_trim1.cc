...
extern int flag11;
extern int count_trim_spec;
extern int max_affec_scc;
extern int flag10;
extern int max_scc;
extern int min_scc;
extern int flag_del;
...
inline static void trim_once_node(gm_graph& G, int curr_color, int& count, node_t n)
{
    ...
    if(met_algo==9 && vec_scc_count[scc_list[n]] == -1)
    {
        G_Color[n] = -2;
        G_SCC[n]=-1;
        count++;
    }
    if(met_algo==7 && affect_level[level_ver[n]]==0)
    {
        G_SCC[n]=n;
        G_Color[n] = -2;
        count++;
