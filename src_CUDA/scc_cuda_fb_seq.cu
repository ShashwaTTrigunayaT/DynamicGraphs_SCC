...
// ======================================================================
// fw_trim_single — Forward BFS (single-threaded in OpenMP)
//
// OpenMP:
...
//   {
//   public:
//       fw_trim_single(gm_graph& _G, int32_t _base_color, int32_t _fw_color,
//                      NODE_SET* _base_set)
...
//       (level-by-level BFS with the same navigator logic)
//       The fw_set size threshold is checked AFTER the BFS:
//       we build the compact set only if count < G_num_nodes * 0.01.
// ======================================================================

// ======================================================================
// bw_trim_single — Backward BFS (single-threaded in OpenMP)
//
// OpenMP:
...
//   {
//   public:
//       bw_trim_single(gm_graph& _G, int32_t _base_color, int32_t _fw_color,
...
//       The bw_set size threshold is checked AFTER the BFS:
//       we build the compact set only if count < G_num_nodes * 0.01.
// ======================================================================

// ======================================================================
// Device helper kernel: find a pivot in a compact set
// Finds the first node in d_set that still has target_color.
// ======================================================================
__global__ void find_pivot_in_set_kernel(
    ...
    int target_color, int* d_pivot)
{
    ...
    node_t n = d_set[tid];
    if (d_Color[n] == target_color) {
        // OpenMP: pivot = *(base_set->begin());
        // Use atomicMin to get the smallest matching node (deterministic)
        atomicMin(d_pivot, (int)n);
    }
}

// ======================================================================
// Device helper kernel: scatter nodes of a single color from a set
// ======================================================================
__global__ void scatter_color_from_set_kernel(
    const int* d_Color,
    const int* d_src_set, int src_size,
    int target_color,
    int* d_dst_set, int* d_pos)
{
    int tid = blockIdx.x * blockDim.x + threadIdx.x;
    int stride = gridDim.x * blockDim.x;

    for (int i = tid; i < src_size; i += stride) {
        node_t n = d_src_set[i];
        if (d_Color[n] == target_color) {
...
// ======================================================================
// Device helper kernel: find pivot by scanning all nodes with base_color
// Used when d_set_nodes == NULL (no compact set available)
// ======================================================================
__global__ void find_pivot_by_color_kernel(
    const int* d_Color,
...
// ======================================================================
// do_fw_bw_single_thread()
//
// OpenMP:
//   int do_fw_bw_single_thread(gm_graph& G, my_work* work,
//                               std::vector<my_work*>& new_works)
//   {
//       if (work->count == 0) return 0;
//       // repeat_local_trim1(G, work);  // COMMENTED OUT in CPU
//       if (work->count == 0) return 0;
//
...
//
//       // BW BFS
//       bw_trim_single BW_BFS(G, base_color, fw_color, bw_color,
    ...
    CUDAMyWork* w, std::vector<CUDAMyWork*>& new_works)
{
    ...
    // OpenMP: // repeat_local_trim1(G, work);  // COMMENTED OUT
    // OpenMP: if (work->count == 0) return 0;
    //         (second check after trim1)
    if (w->count == 0) return 0;
    ...
    int set_capacity = w->set_capacity;
    ...
    if (d_base_set == NULL) {
        ...
        if (w->count == 0) {
            // OpenMP: delete base_set; base_set = NULL;
            //         pivot = choose_pivot_from_color(G, base_color);
    ...
    // OpenMP: if (base_set exists) base_set->erase(k) during BFS
    ...
    // (lazy generation in start_workers_fw_bw will build it when needed).
    ...
    if (fw_count > 0) {
        CUDAMyWork* new_w = new CUDAMyWork();
        new_w->color       = fw_color;
        new_w->count       = fw_count;
        new_w->d_set_nodes = d_fw_set;    // NULL if above threshold (matches OpenMP)
        new_w->set_capacity = (d_fw_set != NULL) ? fw_count : 0;
        new_w->depth       = depth;
        new_w->owns_set    = (d_fw_set != NULL);  // CUDA: track ownership
        new_works.push_back(new_w);
    }
    if (bw_count > 0) {
        CUDAMyWork* new_w = new CUDAMyWork();
        new_w->color       = bw_color;
        new_w->count       = bw_count;
        new_w->d_set_nodes = d_bw_set;    // NULL if above threshold
        new_w->set_capacity = (d_bw_set != NULL) ? bw_count : 0;
        new_w->depth       = depth;
        new_w->owns_set    = (d_bw_set != NULL);
        new_works.push_back(new_w);
    }
    if (base_count > 0) {
        CUDAMyWork* new_w = new CUDAMyWork();
        new_w->color       = base_color;
        new_w->count       = base_count;
        new_w->d_set_nodes = d_base_set_new;  // NULL if above threshold
        new_w->set_capacity = (d_base_set_new != NULL) ? base_count : 0;
        new_w->depth       = depth;
        new_w->owns_set    = (d_base_set_new != NULL);
        new_works.push_back(new_w);
    }
...
//      a. Lazily build compact set if needed (count < 1% of N and no set)
//      b. Call do_fw_bw_single_thread (which launches GPU kernels)
...
void start_workers_fw_bw(GPUState& st, const GPUGraph& g, int N)
{
    ...
    // OpenMP: while (true) { ... }
    while (true) {
        // OpenMP: if (my_works.size() == 0) work_q_fetch_N(...)
        if (my_works.size() == 0) {
            int batch_size = std::max(N / 2, 1);
            work_q_fetch_N(0, batch_size, my_works);
        }
        ...
        while (my_works.size() > 0) {
            ...
            //   if ((w->count < G.num_nodes() * 0.01) && (w->color_set == NULL))
            //       w->color_set = generate_compact_set(G, w->color);
            if ((w->count < num_nodes * 0.01) && (w->d_set_nodes == NULL)) {
                ...
                if (d_trim_targets_count > 0) {
                    ...
                    int gs = (num_nodes + bs - 1) / bs;
                    scatter_single_color_all_nodes_kernel<<<gs, bs>>>(
                        st.d_Color, num_nodes,
                        w->color,
                        d_new_set, d_pos);
                    CUDA_CHECK(cudaDeviceSynchronize());
                }
            ...
            // OpenMP: do_fw_bw_single_thread(G, w, new_works);
            do_fw_bw_single_thread(st, g, w, new_works);
            ...
            //   while ((my_works.size() < N) && (new_works.size() > 0)) {
            //       my_works.push_back(new_works.back()); new_works.pop_back();
            //   }
            while ((my_works.size() < (size_t)N) && (new_works.size() > 0)) {
                my_works.push_back(new_works.back());
                new_works.pop_back();
            }
